...
import { OmpBuddyStorage, DEFAULT_OMP_BUDDY_STATE_DIR } from "./storage.ts";
...
describe("OMP host storage root", () => {
  test("defaults to a distinct host-owned .omp/agent/buddy directory", () => {
    expect(DEFAULT_OMP_BUDDY_STATE_DIR.endsWith(join(".omp", "agent", "buddy"))).toBe(true);
    expect(DEFAULT_OMP_BUDDY_STATE_DIR).not.toBe(DEFAULT_PI_BUDDY_STATE_DIR);
  });

  test("reuses file storage behavior without sharing host state", () => {
    const parent = mkdtempSync(join(tmpdir(), "omp-host-storage-"));
